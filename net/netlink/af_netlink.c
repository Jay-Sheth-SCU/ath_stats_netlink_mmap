			      ...
			      char __user *optval, unsigned int optlen)
{
	...
	unsigned int val = 0;
	int err;
	printk(KERN_DEBUG "[Netlink][af_netlink.c][1]\n");
	if (level != SOL_NETLINK)
		return -ENOPROTOOPT;
	printk(KERN_DEBUG "[Netlink][af_netlink.c][2]\n");
	if (optname != NETLINK_RX_RING && optname != NETLINK_TX_RING &&
	    optlen >= sizeof(int) &&
	    get_user(val, (unsigned int __user *)optval))
		return -EFAULT;
	printk(KERN_DEBUG "[Netlink][af_netlink.c][3]\n");
	switch (optname) {
	...
	case NETLINK_TX_RING: {
		printk(KERN_DEBUG "[Netlink][af_netlink.c][5]\n");
		struct nl_mmap_req req;
